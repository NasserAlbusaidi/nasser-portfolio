...
const ProgressCharts = ({ chartData }) => {
  return (
    <div className="w-full h-full flex flex-col p-6 bg-[#0a0a0a] border border-[#222] rounded-lg shadow-sm font-mono relative overflow-hidden min-h-[400px]">
      ...
      {/* Header */}
      <div className="flex items-center gap-2 mb-4 border-b border-[#222] pb-4">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="2 4" stroke="#1a1a1a" vertical={false} />
             <XAxis dataKey="week" tick={{ fill: '#444', fontSize: 10, fontFamily: 'monospace' }} axisLine={{ stroke: '#222' }} tickLine={false} dy={15} tickFormatter={(value) => value.substring(5)} />
             <YAxis tick={{ fill: '#444', fontSize: 10, fontFamily: 'monospace' }} axisLine={false} tickLine={false} dx={-10} />
             <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#333', strokeWidth: 1, strokeDasharray: '4 4' }} />
             <Legend content={<CustomLegend />} />
             <Line type="monotone" dataKey="bike" stroke={COLORS.bike} strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#000', stroke: COLORS.bike, strokeWidth: 2 }} animationDuration={1500} />
             <Line type="monotone" dataKey="run" stroke={COLORS.run} strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#000', stroke: COLORS.run, strokeWidth: 2 }} animationDuration={1500} />
             <Line type="monotone" dataKey="swim" stroke={COLORS.swim} strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#000', stroke: COLORS.swim, strokeWidth: 2 }} animationDuration={1500} />
             <Line type="monotone" dataKey="workout" stroke={COLORS.workout} strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#000', stroke: COLORS.workout, strokeWidth: 2 }} animationDuration={1500} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
